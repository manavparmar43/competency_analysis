...
class QuestionList(View):
    def get(self, request):
            ...
            if user ==  False:
                ...
                url='http://127.0.0.1:8000/static/json/sentences.json/'
                ...
                return render(request,'question.html',{'data':final_data})
    def post(self,request):
        ...
        if user == False:
            ...
            if a == style_1:
                user_ans_sheet=UserAnswerSheet.objects.create(user=user,answer_key=final_lst,style='style_1')
                user_ans_sheet.save()
                del request.session['user_id']
                ans=" ".join([' '.join([str(c) for c in lst]) for lst in final_lst])
                data={'name':user.name,'position':user.position,'ans':ans,'style':user_ans_sheet.style}
                return render(request,'user_answer_list.html',data)
            elif a == style_2:
                user_ans_sheet=UserAnswerSheet.objects.create(user=user,answer_key=final_lst,style='style_2')
                user_ans_sheet.save()
                del request.session['user_id']
                ans=" ".join([' '.join([str(c) for c in lst]) for lst in final_lst])
                data={'name':user.name,'position':user.position,'ans':ans,'style':user_ans_sheet.style}
                ...
                user_ans_sheet=UserAnswerSheet.objects.create(user=user,answer_key=final_lst,style='style_3')
                user_ans_sheet.save()
                del request.session['user_id']
                ans=" ".join([' '.join([str(c) for c in lst]) for lst in final_lst])
                data={'name':user.name,'position':user.position,'ans':ans,'style':user_ans_sheet.style}
                ...
                user_ans_sheet=UserAnswerSheet.objects.create(user=user,answer_key=final_lst,style='style_4')
                user_ans_sheet.save()   
                del request.session['user_id']
                ans=" ".join([' '.join([str(c) for c in lst]) for lst in final_lst])
                data={'name':user.name,'position':user.position,'ans':ans,'style':user_ans_sheet.style}
